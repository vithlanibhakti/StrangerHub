...
function GetMedia($media, $allow_empty = true) {
    ...
    if($_SERVER['DOCUMENT_ROOT'] == 'C:/WampDeveloper/Websites/localhost/webroot'){
            ...
            if ($mediafiles) {
                ...
                foreach ($mediafiles as $mediafile) {
                    $mf = array(
                        ...
                        'video_file' => $mediafile['video_file'],
...
function GetSiteUsers(){
    ...
    $sql  = mysqli_query($conn, "SELECT * FROM `users`  WHERE `username` != 'guest' and role='vendor';");
    while ($fetched_data = mysqli_fetch_assoc($sql)) {
...
function Wo_UpdateBalance($user_id = 0, $balance = 0, $type = '+') {
    ...
    $user_data = Wo_UserData($user_id);
    if ($type == '+') {
        $balance = ($user_data['aff_balance'] + $balance);
    } else {
        $balance = ($user_data['aff_balance'] - $balance);
    }
...
function SendQueueEmails(){
    ...
    $mail = new PHPMailer;
    $data = array();
    $query_one = " SELECT * FROM `emails` WHERE `src` = 'admin' ORDER BY `id` DESC LIMIT 100";
    $sql       = mysqli_query($conn, $query_one);
    if (mysqli_num_rows($sql) < 1) {
        return false;
    }
...
function is_avatar_approved($user_id, $avatar){
    ...
    $query   = "SELECT `is_approved` FROM `mediafiles` WHERE `user_id` = $user_id AND `file` = '{$avatar}'";
    $sql_query = mysqli_query($conn, $query);
    while ($sql_fetch = mysqli_fetch_assoc($sql_query)) {
        $data = $sql_fetch;
    }
    if(empty($data)){
        return false;
    }else if(isset( $data['is_approved'] ) && $data['is_approved'] == '1'){
        return true;
    }else if(isset( $data['is_approved'] ) && $data['is_approved'] == '0'){
        return false;
    }
}