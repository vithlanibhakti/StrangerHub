...
?>
                ...
                <!-- <a href="<?php echo $site_url;?>/find-matches" data-ajax="/find-matches">upgrade your profile : -->
                 <?php
                   $col=mysqli_query($con,"SELECT amount,pro_plan FROM payments where user_id=$id");
                    ...
                    if($amount == '50')
                    {
            $col=mysqli_query($con,"SELECT COLUMN_NAME FROM INFORMATION_SCHEMA.COLUMNS WHERE TABLE_NAME = 'vendorpremium' and COLUMN_NAME != 'id' and COLUMN_NAME != 'silver' ORDER BY ORDINAL_POSITION");
            while($row2 = mysqli_fetch_assoc($col))
            {
        ...
        elseif($amount == '80')
        {
            $col=mysqli_query($con,"SELECT COLUMN_NAME FROM INFORMATION_SCHEMA.COLUMNS WHERE TABLE_NAME = 'vendorpremium' and COLUMN_NAME != 'id' and COLUMN_NAME != 'silver' and COLUMN_NAME != 'gold' ORDER BY ORDINAL_POSITION");
            while($row2 = mysqli_fetch_assoc($col))
            {
                  $COLUMN_NAME = $row2['COLUMN_NAME'];
                  ?>
