    ...
    <div class="container">
        ...
        <div class="sett_navbar valign-wrapper">
            <ul class="tabs">
                ...
                <li class="tab col s3"><a class="active" href="<?php echo $site_url;?>/settings-sessions/<?php echo $profile->username;?>" data-ajax="/settings-sessions/<?php echo $profile->username;?>" target="_self"><?php echo __( 'Manage Sessions' );?></a></li>
                <li class="tab col s3"><a href="<?php echo $site_url;?>/settings-affiliate/<?php echo $profile->username;?>" data-ajax="/settings-affiliate/<?php echo $profile->username;?>" target="_self"><?php echo __( 'My affiliates' );?></a></li>
...
<div class="container">
    <div class="dt_settings row">
        ...
        <div class="col s12 m8" style="margin: 0 auto;display: block;float: none;">
            <h4><?php// echo __('Manage Sessions');?></h4>
            <table class="highlight responsive-table">
                ...
                foreach ($sessions as $key => $session) {
                    ...
                    echo '<tr class="session_row" data-id="'.$session['id'].'">';
                    echo '  <td>'.$details['platform'].'</td>';
                    echo '  <td>'.$details['name'].'</td>';
                    echo '  <td>'.$session['platform'].'</td>';
                    echo '  <td class="ajax-time" title="'.date('c',$session['time']).'">'.Time_Elapsed_String($session['time']).'</td>';
                    echo '  <td><button class="btn btn-small waves-effect waves-light bold btn_primary btn_round delete_session" type="button" data-id="'.$session['id'].'"><span>'.__('Delete').'</span></button></td>';
                    echo '</tr>';
                }
                ?>
                </tbody>
